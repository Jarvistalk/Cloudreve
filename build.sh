...
buildBinary () {
  cd $REPO
  go build -a -o cloudreve -ldflags " -X 'github.com/Jarvistalk/Cloudreve/v3/pkg/conf.BackendVersion=$VERSION' -X 'github.com/Jarvistalk/Cloudreve/v3/pkg/conf.LastCommit=$COMMIT_SHA'"
}
...
_build() {
    ...
    go build -a -o "${out}" -ldflags " -X 'github.com/Jarvistalk/Cloudreve/v3/pkg/conf.BackendVersion=$VERSION' -X 'github.com/Jarvistalk/Cloudreve/v3/pkg/conf.LastCommit=$COMMIT_SHA'"
